...
public class CartListAdapter extends RecyclerView.Adapter<CartListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.feeEachItem.setText("Ksh" + listFoodSelected.get(position).getFee());
        holder.totalEachItem.setText("Ksh" + Math.round((listFoodSelected.get(position).getNumberInCart() * listFoodSelected.get(position).getFee())));
        ...
        for (int i = 0; i<listFoodSelected.size();i++){
            String titles = listFoodSelected.get(position).getTitle();
